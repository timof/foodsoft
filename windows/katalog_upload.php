...
function upload_terra() {
  ...
  foreach ( $klines as $line ) {
    ...
    if( ! $tag ) {
      ...
      if( preg_match( "#^Art\\.Nr\\.@Bestell-Nr\\.@Obst & GemÃ¼se@Inhalt@Einh\\.@Herk.@HKL@IK@Zertifizierung@@Netto-Preis *@/Einh\\.@MwSt\\.%@Bemerkung@#", $line ) ) {
        /// Art.Nr.@Bestell-Nr.@Obst & GemÃ¼se@Inhalt@Einh.@Herk.@HKL@IK@Zertifizierung@@Netto-Preis @/Einh.@MwSt.%@Bemerkung@@
        $tag='OG';
        $splitat = '@';
        $fields = array( 'anummer', 'bnummer', 'name', 'gebinde', 'einheit', 'herkunft', '', '', 'verband', '',  'netto', '', 'mwst', '' );
        $pattern = '/^[\d\s]+@[\d\s]+@/';
